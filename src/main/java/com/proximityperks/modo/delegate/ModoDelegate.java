...
public class ModoDelegate {
	 ...
	 * @param giftId
	 */
	public void deleteGift(String accountId, String giftId) {
		...
		try {
			...
			String accessToken = getModoToken();
			// construct modo api
			HttpPost post = new HttpPost(BASE_MODO_API + DELETE_GIFT_API);
			List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(1);
			nameValuePairs.add(new BasicNameValuePair(CONSUMER_KEY,
					CONSUMER_KEY_VALUE));
			nameValuePairs
					.add(new BasicNameValuePair(ACCESS_TOKEN, accessToken));
			nameValuePairs.add(new BasicNameValuePair(
					LocationVisitRequest.ACCOUNT_ID, accountId));
			nameValuePairs.add(new BasicNameValuePair("gift_id", giftId));
			post.setEntity(new UrlEncodedFormEntity(nameValuePairs));
			// Execute the modo post
			response = client.execute(post);
			BufferedReader rd = new BufferedReader(new InputStreamReader(
